...
class MapManager: # définit une classe appelée "MapManager" pour gérer la gestion des cartes dans le jeu
    ...
    def __init__(self, mainScreen, player): # définit le constructeur de la classe "MapManager", qui est appelé lorsqu'un nouvel objet "MapManager" est créé. Il prend deux arguments en entrée : "mainScreen", qui représente l'écran principal du jeu, et "player", qui représente le joueur
        ...
        self.current_map = "MapV1"          # attribue à l'attribut self.current_map de l'objet MapManager le nom de la carte actuellement affichée à l'écran, qui est "MapV1" dans cet exemple. Cela permet de garder une référence à la carte actuelle pour la gestion des portails et des téléportations

        self.register_map("MapV1", portals=[ # enregistre des nouvelles cartes dans le gestionnaire de cartes (MapManager)
