...
while running:
    ...
    if menu_accueil.is_active:
        ...
        for event in pygame.event.get():
            evenement.run_choixBase(running, event, choixBase, accueil, pnj, mainScreen, soundDesign, monde_reve)

    elif accueil.is_active:
        accueil.update(mainScreen, pnj, evenement) # On applique les éléments relatif à la page accueil
        for event in pygame.event.get():
            evenement.run_accueil(running, event, pnj, accueil, quete3, soundDesign)

    elif menu_quetes.is_active:
        menu_quetes.update(mainScreen) # On applique les éléments relatif à la page menu_quetes
        for event in pygame.event.get():
            evenement.run_menu_quetes(running, event, pnj, menu_quetes, menu_accueil, descri_quete1, descri_quete2, soundDesign)
        ...
        monde_reve.run(running, quete3, menu_quetes, accueil, pnj, evenement, soundDesign)
