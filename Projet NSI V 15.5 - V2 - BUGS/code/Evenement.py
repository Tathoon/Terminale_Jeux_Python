...
class evenement:
    ...
    def run_choixBase(self, running, event, choixBase, accueil, pnj, mainScreen, soundDesign, monde_reve):
        self.gerer_quitter(event, running)
    ...
    def run_accueil(self, running, event, pnj, accueil, quete3, soundDesign):
        self.gerer_quitter(event, running)
        self.retour(event, accueil, "choixBase", None, soundDesign)
        self.transit_page(None, None, accueil, "accueil", quete3, "quete3", pnj, soundDesign)
        if event.type == pygame.KEYDOWN or event.type == pygame.KEYUP:
            self.touches_pressees(event, pnj)
    
    def run_menu_quetes(self, running, event, pnj, menu_quetes, menu_accueil, descri_quete1, descri_quete2, soundDesign):
        self.gerer_quitter(event, running)
        self.retour(event, menu_quetes, "quete3", pnj, soundDesign)
    ...
    def transit_page(self, event, bouton, page_actu, page_actu_nom, page_suiv, page_suiv_nom, pnj, soundDesign):
        if page_actu_nom == "accueil":
            if pnj.type.rect.x < -15:
                ...
                soundDesign.play_music("quete3", 1, -1)
                soundDesign.play_music("quete3", 1, -1)
            ...
            if page_actu_nom == "menu_accueil":
                ...
                if bouton.rect.collidepoint(event.pos):
                    ...
                    pygame.mixer.music.stop()
                    soundDesign.play_music("quete1", 1, -1)
                    self.pile_retour.empiler(page_actu)
                    self.pile_retour.empiler(page_suiv)
                ...
                if bouton.rect.collidepoint(event.pos):
                    ...
                    pygame.mixer.music.stop()
                    soundDesign.play_music("quete2", 1, -1)
                    self.pile_retour.empiler(page_actu)
                    self.pile_retour.empiler(page_suiv)
    ...
    def transit_Quete1_Defaite(self, pnj, quete1, defaite, soundDesign):
        quete1.is_active = False
        defaite.is_active = True
        pnj.all_monster.empty()
        pnj.type.health = pnj.type.max_health
        pygame.mixer.music.stop()
        soundDesign.play_music("game_over", 1, 0)
