...
class monde_reve:
    ...
    def type(self, pnj):
        ...
        # generer un joueur 
        if pnj.type_name == "Wizard":
            self.player = Player("Wizard", 16, 32, 32, 0, 96, 64, 0, 3, 16)     # creer un objet de la classe "player" appelé "Wizard"
        elif pnj.type_name == "Swordman":
            self.player = Player("Swordman", 32, 48, 144, 0, 96, 48, 0, 4, 32)  # creer un objet de la classe "player" appelé "Swordman"
...
class Entity(AnimateSprite): # définit une nouvelle classe appelée "player_reve" qui hérite de la classe "pygame.sprite.Sprite"
    ...
    def __init__(self, name, x , y, x1, y1, up, down, right, left, begincut, cut, nbcut):
        super().__init__(name, x1, y1, up, down, right, left, begincut, cut, nbcut)                      # appelle la méthode "init()" de la classe mère "pygame.sprite.Sprite", cela permet d'initialiser l'objet comme un sprite pygame
        self.image = self.get_image(0, 0, x1, y1)    # appelle la méthode "get_image()" de l'objet avec les coordonnées x=0 et y=0 pour obtenir une sous-image de l'attribut "sprite_sheet" et l'assigne à l'attribut "image" de l'objet
...
class Player(Entity):
    ...
    def __init__(self, name, x1, y1, up, down, right, left, begincut, cut, nbcut):
        super().__init__(f"{name}", 0, 0, x1, y1, up, down, right, left, begincut, cut, nbcut)