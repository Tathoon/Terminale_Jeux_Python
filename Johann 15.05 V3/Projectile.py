...
class projectile(pygame.sprite.Sprite):
    """
    ###attributs:
        - speed : vitesse de déplacement du projectile
        - image : représentation iconographique
        - rect : image sous forme objet Rect
        - rect.x : coordonnées en x du projectile
        - rect.y : coordonnées en y du projectile
        - origin_image : imgae initiale
        - angle : angle de rotation du projectile

    ###méthodes:
        - rotate -> fait tourner le projectile (stylé)
        - remove -> TUE le projectile (le supprime)
        - move -> gère le déplacement du projectile et ses dégats
