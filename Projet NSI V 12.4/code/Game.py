...
class game:
    ...
    def monster_vers_joueurs(self, monster, pnj, evenement, quete1, defaite):
        # Le déplacement ne se fait que si il n'y a pas de collision avec un monstre
        if not self.check_collision(self.type, self.all_monster):
            monster.rect.x -= monster.speed
        # Si le monstre est en collision avec le joueur:
        else:
            pnj.damage(monster.attack, evenement, quete1, defaite)
    ...
    def damage(self, amount, evenement, quete1, defaite):
        if self.type.health - amount > amount:
            self.type.health -= amount
        else: 
            quete1.is_active, defaite.is_active = evenement.transition_Quete1_Defaite(quete1, defaite)
